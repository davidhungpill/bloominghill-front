...
**Hero 이미지 추가 필드** (소개·사업 메뉴):

| 필드명 | 타입 | 대상 페이지 |
|---|---|---|
| `heroIntro` | Media (single) | 소개 6개 메뉴 공용 (이사장 인사말~오시는 길) |
| `heroOrchestra` | Media (single) | 오케스트라 |
| `heroNanum` | Media (single) | 이웃사랑 나눔사업 |
| `heroScholarship` | Media (single) | 장학사업 |

**통계 수치 필드** (HomeView Quick Stats — 단위 포함 텍스트):

| 필드명 | 타입 | 예시 | 위치 |
|---|---|---|---|
| `statOrchestra` | Short text | `128회` | 오케스트라 정기공연 |
| `statScholarship` | Short text | `542명` | 장학생 누적 지원 |
| `statDonation` | Short text | `15.2억` | 나눔 사업 누적 기부 |

**후원 계좌 필드** (DonateView):

| 필드명 | 타입 | 예시 |
|---|---|---|
| `bankName` | Short text | 농협은행 |
| `accountNumber` | Short text | 301-1234-5678-01 |
| `accountHolder` | Short text | 사단법인 꽃재 |
| `phone` | Short text | 02-2299-5424 |

**기부금 영수증 문의 필드** (DonateCertView):

| 필드명 | 타입 | 예시 |
|---|---|---|
| `receiptEmail` | Email | blooming5424@naver.com |
| `kakaoChannel` | Short text | @꽃재복지재단 |
| `kakaoChannelUrl` | Short text | https://pf.kakao.com/... |
...
- [ ] **SiteConfig Single Type** (Hero 11개 + 통계 3개 + 계좌 4개 + 기부금문의 3개 = 총 21개 필드)
